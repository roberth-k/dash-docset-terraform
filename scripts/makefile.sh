...
do
    ...
    output_file="$dstdir/Contents/Resources/Documents/terraform/$(realpath -m --relative-to $terraform_docs_dir $(dirname $input_file))/$barename.html"
    ...
    echo "	@mkdir -p \$(dir \$@)"
    echo "	render.py --in \$< --out \$@ --docset $(realpath -m $dstdir)"
done
